...
function deal() {
  ...
  if (deck[12].suit === "♦" || deck[12].suit === "♥") {
    deck[12].el.classList.add("red");
  }
  cutCard = deck[12];
  cutCard.cut = true;
  ...
  if (whoStarts == true) {
    whoseCrib.innerHTML = "Computer's crib";
  } else {
    whoseCrib.innerHTML = " Your crib";
  }
}
if (round == 1) {
  makeDeck();
...
function cut(e) {
  ...
  if (deck[x].suit === "♦" || deck[x].suit === "♥") {
    aCut.classList.add("red");
  }
...
function compCut(e) {
  ...
  if (deck[y].suit === "♦" || deck[y].suit === "♥") {
    bCut.classList.add("red");
  }
  deck[y].el = bCut;
  start.append(bCut);
  ok = document.createElement("button");
...
function whoGoesFirst(e) {
  if (deck[y].rank < deck[x].rank) {
    start.innerHTML =
      "The computer's card is lower, so it has the crib first. Select 2 cards to put in the crib. Select a card by clicking it. To deselect, click again.";
    whoStarts = true;
  } else {
    start.innerHTML =
      "Your card is lower, so it's your crib first. Select 2 cards to put in the crib. Select a card by clicking it. To deselect, click again.";
    whoStarts = false;
  }
  ok = document.createElement("button");
  ok.classList.add("popupbutton");
...
function playerMove(e) {
  let selected = playerPeg.filter((obj) =>
    obj.el.classList.contains("selected")
  );
  if (selected.length > 0 && canPlay(selected)) {
    ...
    if (!canPlay(compPeg) && cCount < 4 && runningTotal != 31) {
      ...
      peggingUpdates.innerHTML = "Computer says go";
...
async function compMove() {
  ...
  if (!canPlay(playerPeg) && canPlay(compPeg)) {
    compMove;
  }
}
  ...
  function runs() {
    ...
    if (noduplicates.length > 2) {
      for (i = 0; i < noduplicates.length - 2; i++) {
        if (
          noduplicates[i + 2] - noduplicates[i + 1] == 1 &&
          noduplicates[i + 1] - noduplicates[i] == 1
        ) {
          seqPoints++;
      ...
      if (seqPoints > 0) {
        runPoints = (seqPoints + 2) * multiplier;
      }
    } else {
      runPoints = 0;
    }
...
function get31() {
  if (runningTotal == 31) {
    ...
    info.innerHTML = runningTotal;
    if (Pgo || Cgo) {
      peggingUpdates.innerHTM += " *1 for go";
      return 3;
    }
...
function getPair() {
  ...
  for (i = discard.length - 1; i > -1; i--) {
    if (
      discard[i - 1] != undefined &&
      discard[discard.length - 1].rank != discard[discard.length - 2].rank
    ) {
      return 0;
    }
  ...
  if (pairCount > 0) {
    peggingUpdates.innerHTML += " *" + (pairCount + 1) + " of a kind";
  }
  if (pairCount == 0) {
    return 0;
  }
...
function canPlay(a) {
  ...
  if (playableIndices.length > 0 && played < 4) {
    return true;
  } else {
    return false;
  }
...
async function countPlayerHand() {
  if (playerScore < 121 && compScore < 121) {
    ...
    if (playerScore < 121 && compScore < 121) {
      ...
      notiBox.appendChild(start);
      start.innerHTML =
        "Your hand is worth " + calculate(newPlayerHand)[5] + ".";
...
async function countCompHand() {
  if (playerScore < 121 && compScore < 121) {
    ...
    if (compScore < 121 && playerScore < 121) {
      notiBox.appendChild(start);
      start.innerHTML =
        "The computer's hand is worth " + calculate(newCompHand)[5] + ".";
      ...
      if (counted == 1) {
        ok.addEventListener("click", countPlayerHand);
      }
      if (counted == 2) {
        ok.addEventListener("click", countCrib);
      }
...
function playerPegMove() {
  if (playerScore != 0) {
    ...
    if (playerScore % 60 != 0) {
      newSpot = playerScore % 60;
    } else {
      newSpot = 60;
    }
    if (pOldScore % 60 != 0) {
      oldSpot = pOldScore % 60;
    } else {
      oldSpot = 60;
    }
    ...
    for (i = 1; i < 61; i++) {
      if (
        p1PegArr[i].el.classList.contains("occupied") &&
        i != newSpot &&
        i != oldSpot
      ) {
...
function compPegMove() {
  if (compScore != 0) {
    ...
    let oldSpot;
    if (compScore % 60 != 0) {
      newSpot = compScore % 60;
    } else {
      newSpot = 60;
    }
    if (cOldScore % 60 != 0) {
      oldSpot = cOldScore % 60;
    } else {
      oldSpot = 60;
    }
    ...
    for (i = 1; i < 61; i++) {
      if (
        p2PegArr[i].el.classList.contains("occupied2") &&
        i != newSpot &&
        i != oldSpot
      ) {
